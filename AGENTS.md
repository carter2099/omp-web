...
`GET /api/models` returns `defaultModel` from OMP Settings under `~/.omp/agent` (config.yml overlays). `ChatWindow` pre-selects this on mount for new sessions.
...
- `ModelsConfig` combines models from `~/.omp/agent/models.yml` with provider auth status from OMP `AuthStorage`/`ModelRegistry` (via `getOmpRuntime`).
