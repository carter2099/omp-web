...
test("OMP presets use glob not find/ls and include task", () => {
  assert.deepEqual(PRESET_DEFAULT, ["read", "bash", "edit", "write", "task"]);
  assert.ok(PRESET_FULL.includes("glob"));
  assert.ok(PRESET_FULL.includes("task"));
...
test("empty / none preset excludes task", () => {
  assert.deepEqual(getToolNamesForPreset("none"), []);
  assert.ok(!getToolNamesForPreset("none").includes("task"));
});

test("default and full presets include task", () => {
  assert.ok(getToolNamesForPreset("default").includes("task"));
  assert.ok(getToolNamesForPreset("full").includes("task"));
  assert.deepEqual(
    [...getToolNamesForPreset("default")].sort(),
    [...PRESET_DEFAULT].sort(),
  );
  assert.deepEqual(
...
test("getPresetFromTools recognizes full/default with task present", () => {
  const tools = [
  ...
  // Extension/custom tools co-active with the full builtin set still map to full
  // (task is a builtin in PRESET_FULL so it participates in equality).
  const fullPlusCustom = [
    ...tools,
    { name: "my-extension", description: "ext", active: true },
  ];
  assert.equal(getPresetFromTools(fullPlusCustom), "full");
});
