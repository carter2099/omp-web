      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
        ...
        <div ref={topBarRef} style={{ display: "flex", alignItems: "center", flexShrink: 0, borderBottom: "1px solid var(--border)", height: 36, background: "var(--bg-panel)" }}>
          ...
          {showChat && (
            <div style={{ display: "flex", alignItems: "stretch", height: "100%" }}>
              <button
                ...
                title={selectedSession ? "View full history" : "History available after session is saved"}
              ...
              {(() => {
                ...
                const title = !selectedSession
                  ? "Name available after session is saved"
                  : !hasMessages
                    ? "Send a message first to name the session"
                    : isError
          ...
          {showChat && (sessionStats || contextUsage) && (() => {
            ...
            if (t) {
              tooltipParts.push(`Input: ${t.input.toLocaleString()}`);
              tooltipParts.push(`Output: ${t.output.toLocaleString()}`);
            ...
            if (contextUsage?.contextWindow) {
              const pct = contextUsage.percent;
              tooltipParts.push(`Context: ${pct !== null ? pct.toFixed(1) + "%" : "?"} of ${contextUsage.contextWindow.toLocaleString()} tokens`);
            }
                ...
                <div style={{
                  ...
                  {systemPrompt ? (
                    ...
                    <div style={{ padding: "10px 16px", fontSize: 12, color: "var(--text-muted)", fontStyle: "italic" }}>
                      System prompt is empty (tools disabled)
                    </div>
                  ) : (
                    <div style={{ padding: "10px 16px", fontSize: 12, color: "var(--text-muted)", fontStyle: "italic" }}>
                      System prompt loaded after sending a message
                    </div>
                  )}
                </div>
              )}
              ...
              <div style={{ position: "absolute", top: 12, left: 12, display: "flex", alignItems: "flex-start", gap: 8, userSelect: "none", pointerEvents: "none" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "var(--text-muted)", lineHeight: 1.8 }}>
                    ...
                    <span style={{ color: "var(--text-dim)", marginRight: 6 }}>2.</span>Add a model via the <strong style={{ color: "var(--text)" }}>Models</strong> button at the bottom
                  </div>
                </div>
              </div>
            )
